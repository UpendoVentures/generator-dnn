...
module.exports = class extends Generator {
  ...
  prompting() {
    const prompts = [ {
        ...
        type:'input', 
        name:'companyName', 
        message:'What is your company name? (Do not include spaces or punctuation, e.g., UpendoVentures)', 
        store:true, 
        validate:str =>  {
          return str.length > 0; 
        }
  ...
  writing() {
    ...
    this.fs.copy(
      ...
      this.destinationPath('DotNetNuke.log4net.config')
    ); 
  ...
  install() {
    this.log(chalk.white('Creating solution folders in file system.')); 
    mkdirp.sync('Assets'); 
    mkdirp.sync('Libraries'); 
    mkdirp.sync('Modules'); 
    mkdirp.sync('References'); 
